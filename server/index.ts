...
app.post(
  ...
  async (c) => {
    ...
    if (attempts >= 10) {
        ...
        throw new HTTPException(500, { message: 'Internal Server Error: Could not generate unique access code' });
    }
    ...
    try {
      ...
      return c.json({
        ...
        throw new HTTPException(500, { message: 'Internal Server Error: Failed to save challenge' });
    }
  }
);
...
app.post(
  ...
  async (c) => {
    ...
    let participant = challenge.participants.find(p => p.deviceId === payload.deviceId);
    ...
    if (!participant) {
      ...
      try {
        store.updateChallenge(challenge); // Save the updated challenge with the new participant
         ...
         throw new HTTPException(500, { message: 'Internal Server Error: Failed to save participant' });
      }
    } else {
        // Optional: Update display name if participant rejoins with a different name?
        // For now, we just recognize them.
        console.log(`Participant ${participant.id} (${payload.deviceId}) rejoined challenge ${challenge.id}`);
    }
...
app.post(
  ...
  async (c) => {
    ...
    const participant = challenge.participants.find(p => p.id === payload.participantId);

    if (!participant) {
        throw new HTTPException(404, { message: 'Participant not found in this challenge' });
    }

    // Verify the device ID matches the participant submitting the score
    if (participant.deviceId !== payload.deviceId) {
        console.warn(`Device ID mismatch for participant ${payload.participantId}. Expected ${participant.deviceId}, got ${payload.deviceId}`);
        throw new HTTPException(403, { message: 'Forbidden: Device ID mismatch' });
    }
    ...
    */
    ...
    try {
      ...
      return c.json(newAttempt, 201); // Return the created attempt
       ...
       throw new HTTPException(500, { message: 'Internal Server Error: Failed to save attempt' });
    }
  }
);
...
app.get('/api/challenges/:id/leaderboard', (c) => {
  ...
  const leaderboard: LeaderboardEntry[] = challenge.participants.map(participant => {
