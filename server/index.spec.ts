...
const createValidAttemptPayload = (participant: Participant, score: number, overrides = {}): SubmitAttemptPayload => ({
...
describe('Challenge API', () => {
  ...
  describe('POST /api/challenges', () => {
    it('should create a new challenge successfully with valid data', async () => {
      ...
      if (storedChallenge) { // Type guard for challenge
        ...
        if (participant) { // Type guard for participant
    ...
    it('should return 400 if name is missing', async () => {
      ...
      if (body.errors) { // Type guard
        expect(body.errors).toHaveProperty('name');
      }
    });

     it('should return 400 if settings are invalid', async () => {
      const payload = createValidPayload({ settings: { precision: -1, maxLife: 3, gameType: 'Color' } }); // Invalid precision
      const req = new Request('http://localhost/api/challenges', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      ...
      expect(res.status).toBe(400);
       ...
       if (body.errors) { // Type guard
         ...
         const hasSettingsError = Object.keys(body.errors).some(key => key.startsWith('settings'));
  ...
  describe('POST /api/challenges/join', () => {
    ...
    it('should allow a new participant to join successfully', async () => {
        ...
        const joiner = body.participants.find(p => p.deviceId === 'new-device-2');
        expect(joiner).toBeDefined();
        if (joiner) { // Add type guard
    ...
    it('should return 400 if deviceId is missing', async () => {
        const joinPayload = { // Intentionally missing deviceId
            accessCode: createdChallenge.accessCode,
  ...
  describe('POST /api/challenges/:id/attempts', () => {
    ...
    beforeEach(async () => {
        ...
        if (!initialChallengeState || initialChallengeState.participants.length === 0) {
            throw new Error("Test setup failed: Creator participant not found after challenge creation.");
        }
        ...
        const foundJoiner = updatedChallengeState.participants.find(p => p.deviceId === 'joiner-device-id');
        expect(foundJoiner).toBeDefined();
        if (!foundJoiner) {
             throw new Error("Test setup failed: Joiner participant not found after joining.");
        }
    ...
    it('should allow creator to submit a valid attempt', async () => {
        ...
        const req = new Request(`http://localhost/api/challenges/${createdChallenge.id}/attempts`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (storedChallenge && storedChallenge.attempts.length > 0) {
            ...
            if (attempt) { // Type guard for attempt
                expect(attempt.id).toBe(body.id);
                expect(attempt.score).toBe(100);
            }
        } else {
            // Fail test explicitly if attempt wasn't stored correctly
            throw new Error("Attempt verification failed: Attempt not found in store or store is empty.");
        }
    });
    ...
    it('should return 400 for invalid score', async () => {
     ...
     it('should return 400 for missing metadata', async () => {
        const attemptPayload = { // Missing metadata
    ...
    it('should return 404 for non-existent challenge ID', async () => {
        const attemptPayload = createValidAttemptPayload(creatorParticipant, 100);
        const req = new Request('http://localhost/api/challenges/invalid-challenge-id/attempts', { // Bad ID - Use simple string
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(attemptPayload),
        });
  ...
  describe('GET /api/challenges/:id/leaderboard', () => {
    ...
    beforeEach(async () => { // Mark as async
        ...
        const initialChallengeState = store.getChallengeById(createdChallenge.id);
        if (!initialChallengeState || initialChallengeState.participants.length === 0) throw new Error("Setup failed: Creator not found");
        const creatorP = initialChallengeState.participants[0]; // Use temp var
        expect(creatorP).toBeDefined(); // Explicitly check participant exists
        if (!creatorP) throw new Error("Setup failed: Creator participant is undefined");
        ...
        const joinPayload1: JoinChallengePayload = { accessCode: createdChallenge.accessCode, deviceId: 'joiner1-dev', displayName: 'Joiner One' };
        const joinReq1 = new Request('http://localhost/api/challenges/join', {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(joinPayload1),
        });
        const joinRes1 = await app.request(joinReq1);
        expect(joinRes1.status).toBe(200);
        const state1 = await joinRes1.json() as Challenge;
        const foundJoiner1 = state1.participants.find(p => p.deviceId === 'joiner1-dev');
        if (!foundJoiner1) throw new Error("Setup failed: Joiner 1 not found");
        joiner1 = foundJoiner1;

        // 3. Join participant 2
        const joinPayload2: JoinChallengePayload = { accessCode: createdChallenge.accessCode, deviceId: 'joiner2-dev', displayName: 'Joiner Two' };
        const joinReq2 = new Request('http://localhost/api/challenges/join', {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(joinPayload2),
        });
        const joinRes2 = await app.request(joinReq2);
        expect(joinRes2.status).toBe(200);
        const state2 = await joinRes2.json() as Challenge;
        const foundJoiner2 = state2.participants.find(p => p.deviceId === 'joiner2-dev');
    ...
    it('should return the leaderboard sorted by score (desc)', async () => {
        const req = new Request(`http://localhost/api/challenges/${createdChallenge.id}/leaderboard`, {
            method: 'GET',
        });
        const res = await app.request(req);
        expect(res.status).toBe(200);
     ...
     it('should return an empty leaderboard if no attempts submitted', async () => {
        ...
        const req = new Request(`http://localhost/api/challenges/${emptyChallenge.id}/leaderboard`, {
            method: 'GET',
        });
        const res = await app.request(req);
        expect(res.status).toBe(200);
    ...
    it('should return 404 for non-existent challenge ID', async () => {
         const req = new Request('http://localhost/api/challenges/invalid-id/leaderboard', {
            method: 'GET',
        });
        const res = await app.request(req);
        expect(res.status).toBe(404);
